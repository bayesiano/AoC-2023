...
//        log { "hashes=$hashes" }
    ...
    private fun solveProblem2(input: String): Int {
        ...
        input.split(",").forEach { op ->
            ...
            if( op.contains('-')) box.removeIf { l -> l.label == label}
            else box.indexOfFirst{ it.label == label}.let { i ->
                if (i >= 0) box[i].focalLength = focalLength.toInt()
                else box += Lens(label, focalLength.toInt())
            }
        }
